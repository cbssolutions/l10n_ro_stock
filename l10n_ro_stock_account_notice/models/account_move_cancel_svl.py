        ...
        for move in self:
            if ( move.is_l10n_ro_record ):
                if move.stock_valuation_layer_ids:
                    svls = move.stock_valuation_layer_ids.filtered(lambda r: not r.l10n_ro_draft_svl_id and not (r.l10n_ro_draft_svl_ids and r.quantity==0))
                    pickings = svls.stock_move_id.picking_id
                    posted_bills_for_notice = pickings.l10n_ro_notice_bill_id.filtered(lambda r: r.state == "posted")
                    if posted_bills_for_notice:
                        raise UserError(
                            _(f"You can not set to draft this entry=({move.id, move.name}),  "
