    ...
    def button_validate(self):
        for rec in self:
            if rec.company_id.l10n_ro_accounting:
                ...
                if not move_lines:
                    raise UserError(_(f"For landed cost=({rec.id},{rec.name}) you do not have stock_move_lines to distribute the cost"))
                svl_without_remaining_qty = move_lines.stock_valuation_layer_ids.filtered(lambda r: not r.remaining_qty)
                if svl_without_remaining_qty:
                    raise UserError(_(f"For landed cost=({rec.id},{rec.name}) you do not svl_without_remaining_qty={svl_without_remaining_qty}"))
